                               ...
                               prte_node_t *node)
{
    ...
    if (PRTE_PROC_STATE_FAILED_TO_START == proc->state ||
        ...
        output = pmix_show_help_string("help-prun.txt", "prun:proc-exit-no-sync", true,
                                       ...
                                       prte_tool_basename);
        return output;
    } else if (PRTE_PROC_STATE_KILLED_BY_SHRINK == proc->state) {
        output = pmix_show_help_string("help-prun.txt", "prun:proc-killed-by-shrink", true,
                                       prte_tool_basename, (unsigned long) proc->name.rank,
...

{
    ...
    for (i = 0; i < job->procs->size; i++) {
        ...
        if (PRTE_PROC_STATE_FAILED_TO_START == pptr->state ||
            ...
            ++num_killed;
        } else if (PRTE_PROC_STATE_KILLED_BY_SHRINK == pptr->state) {
            ++num_killed;
        }
    }
