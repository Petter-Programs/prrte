...
keep_going:
    ...
    /* A DVM shrink released this proc's node and the departing daemon killed
     * it.  Planned for the DVM, not for the job, so it is handled like the
     * loss of a daemon - except that the cause is known and can be named. */
    case PRTE_PROC_STATE_KILLED_BY_SHRINK:
        ...
        if (flag) {
            ...
            if (!PRTE_FLAG_TEST(jdata, PRTE_JOB_FLAG_ABORTED)) {
                jdata->state = PRTE_JOB_STATE_KILLED_BY_SHRINK;
                /* point to the first rank to cause the problem */
                prte_set_attribute(&jdata->attributes, PRTE_JOB_ABORTED_PROC, PRTE_ATTR_LOCAL, pptr,
                                   PMIX_POINTER);
                /* retain the object so it doesn't get free'd */
