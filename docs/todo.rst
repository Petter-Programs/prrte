...
**``ras/pmix``.**  It now gets the DVM's allocation from the scheduler, grows
by a new allocation per ``PMIX_ALLOC_EXTEND`` and returns released nodes from
``shrink_complete``.  Still missing: there is no PMIx directive for "the
requester's own allocation", so startup sends a ``PMIX_ALLOC_NEW`` with no
resources; ``release_allocation`` is not implemented; and PRRTE's own
reservations still go through the old path, which forwards a release before
its shrink drains.
