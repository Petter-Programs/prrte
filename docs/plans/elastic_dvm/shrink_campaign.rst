...
``PRTE_PROC_STATE_KILLED_BY_SHRINK``.  Reported as ``PRTE_PROC_STATE_TERMINATED``
...
* errmgr/dvm handles it as a proc failure.  A **recoverable** or **continuous**
  job is notified (``PMIX_ERR_PROC_KILLED_BY_CMD``) and keeps running with the
  proc's resources recovered; any other job is flagged
  ``PRTE_JOB_STATE_KILLED_BY_SHRINK``, given a non-zero exit code, and
  terminated.
* ``PMIX_JOB_TERM_STATUS`` becomes ``PMIX_ERR_JOB_CANCELED`` and the
  ``prun:proc-killed-by-shrink`` help text names the first rank and its node.
